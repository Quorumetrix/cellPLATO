...
# Previously:
# from combine_compare import get_experiments, combine_dataframes
# from data_processing import get_data_matrix, do_tsne
...
def compare_mig_shape_factors(df_in,dr_factors_mig,dr_factors_shape, dr_factors_all,perp=TSNE_PERP,umap_nn=UMAP_NN, min_dist=UMAP_MIN_DIST):
    ...
    '''
    PLOT THEM,
    Consider transfering to function in visualization
    '''
    # Since each set of PCs will be different, makes sense to use PC1 for each.
    # fac1 = colormap_pcs(dr_df_mig, cmap='rgb') #'PC1'
    # fac2 = colormap_pcs(dr_df_shape, cmap='rgb') #'PC1'
    # fac3 = colormap_pcs(dr_df_all, cmap='rgb') #'PC1'
    tag = '_mig_shape_all_new_'
    ...
    # if(fac3) == 'label':
    #     ax7.scatter(x=dr_df_mig['tSNE1'],y=dr_df_mig['tSNE2'], s=0.5,
    #                 c=dr_df_mig['labels'],cmap='tab20c')
    #     ax8.scatter(x=dr_df_shape['tSNE1'],y=dr_df_shape['tSNE2'], s=0.5,
    #                 c=dr_df_shape['labels'],cmap='tab20c')
    #     ax9.scatter(x=dr_df_all['tSNE1'],y=dr_df_all['tSNE2'], s=0.5,
    #                 c=dr_df_all['labels'],cmap='tab20c')
    #
    # else:
    #     ax7.scatter(x=dr_df_mig['tSNE1'],y=dr_df_mig['tSNE2'], s=0.5,
    #                 c=dr_df_mig[fac3],cmap='cool')
    #     ax8.scatter(x=dr_df_shape['tSNE1'],y=dr_df_shape['tSNE2'], s=0.5,
    #                 c=dr_df_shape[fac3],cmap='cool')
    #     ax9.scatter(x=dr_df_all['tSNE1'],y=dr_df_all['tSNE2'], s=0.5,
    #                 c=dr_df_all[fac3],cmap='cool')
    ...
    #
    # plt.clf()
    # main_title =  'Compare UMAP input groups,  '
    # subtitle = ' cmapping: ' + fac1 + ', ' + fac2
    # fig2, ((ax1, ax2, ax3), (ax4, ax5, ax6), (ax7, ax8, ax9)) = plt.subplots(3, 3, figsize=(15,15))
    # fig2.suptitle(main_title+subtitle,fontsize=20)
    #
    # ax1.set_title('Migration UMAP')
    # ax2.set_title('Cell shape UMAP')
    # ax3.set_title('Ensemble UMAP')
    #
    # ax1.scatter(x=dr_df_mig['UMAP1'],y=dr_df_mig['UMAP2'], s=0.5, c=dr_df_mig[fac1], cmap='viridis')
    # ax2.scatter(x=dr_df_shape['UMAP1'],y=dr_df_shape['UMAP2'], s=0.5, c=dr_df_shape[fac1], cmap='viridis')
    # ax3.scatter(x=dr_df_all['UMAP1'],y=dr_df_all['UMAP2'], s=0.5, c=dr_df_all[fac1], cmap='viridis')
    #
    # ax4.scatter(x=dr_df_mig['UMAP1'],y=dr_df_mig['UMAP2'], s=0.5, c=dr_df_mig[fac2], cmap='plasma')
    # ax5.scatter(x=dr_df_shape['UMAP1'],y=dr_df_shape['UMAP2'], s=0.5, c=dr_df_shape[fac2], cmap='plasma')
    # ax6.scatter(x=dr_df_all['UMAP1'],y=dr_df_all['UMAP2'], s=0.5, c=dr_df_all[fac2], cmap='plasma')
    #
    # if(fac3) == 'label':
    #     ax7.scatter(x=dr_df_mig['UMAP1'],y=dr_df_mig['UMAP2'], s=0.5,
    #                 c=dr_df_mig['labels'],cmap='tab20c')
    #     ax8.scatter(x=dr_df_shape['UMAP1'],y=dr_df_shape['UMAP2'], s=0.5,
    #                 c=dr_df_shape['labels'],cmap='tab20c')
    #     ax9.scatter(x=dr_df_all['UMAP1'],y=dr_df_all['UMAP2'], s=0.5,
    #                 c=dr_df_all['labels'],cmap='tab20c')
    #
    # else:
    #     ax7.scatter(x=dr_df_mig['UMAP1'],y=dr_df_mig['UMAP2'], s=0.5,
    ...
    #
    # if STATIC_PLOTS:
    #     fig2.savefig(DR_PARAMS_DIR+'umap_'+str(umap_nn)+'_mig_shape.png', dpi=300)
...
def sweep_tsne_umap(df_in, perp_min_max=(10,300), nn_min_max=(2, 100),n_vals=11,nested_dbscan_sweep=False, dr_factors=DR_FACTORS, eps_vals=None):
    ...
    if(nested_dbscan_sweep):
        print('Also finished DBscan cluster sweep, see output in directory: ',CLUST_PARAMS_DIR)
